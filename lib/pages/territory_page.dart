  ...
  return FutureBuilder<List<Map>>( // Usa FutureBuilder para la lista de posiciones
    ...
    builder: (context, snapshot) {
      ...
      if (snapshot.connectionState == ConnectionState.waiting) {
        ...
        listaPosiciones.add({"latitud":_japanPlex.latitude, "longitud":_japanPlex.longitude});
        listaPosiciones.add({"latitud":_pomasquiPlex.latitude, "longitud":_pomasquiPlex.longitude});
        // Crea una lista de marcadores vacía
        List<Marker> markersList = [];
        List<LatLng> polylineCoordinates = listaPosiciones
          .map((posicion) => LatLng(posicion["latitud"] as double, posicion["longitud"] as double))
          .toList();
