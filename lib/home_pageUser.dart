      ...
      if (user != null) {
        QuerySnapshot query = await FirebaseFirestore.instance
            ...
            .where('userEmail',
                isEqualTo: user.email
                    ?.toLowerCase()) // Convertir el correo a minúsculas
            .get();
        ...
        if (query.docs.isNotEmpty) {
          ...
          await FirebaseFirestore.instance.collection("posiciones").add({
            ...
            'longitud': position.longitude,
            'userEmail':
                user.email?.toLowerCase(), // Convertir el correo a minúsculas
          });
  ...
  void removeUserFromRealtime(String userMail) async {
    try {
      if (userMail.isNotEmpty) {
        ...
        final query =
            logedUsersReference.orderByChild("logedUserMail").equalTo(userMail);
          ...
          Padding(
              ...
              child: ElevatedButton.icon(
                onPressed: () {
                  ...
                  Workmanager().registerPeriodicTask("1", "simpleTaskLocation",
